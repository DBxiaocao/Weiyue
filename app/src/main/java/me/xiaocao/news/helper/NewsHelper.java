    ...
    public void getNewsDetail(final WangYiDetailRequest detailRequest) {
        HttpUtils.async(detailRequest).map(new Function<ResponseResult, ReadDetail>() {
            ...
            public ReadDetail apply(@NonNull ResponseResult responseResult) throws Exception {
                if (responseResult.success) {
                    Map<String, ReadDetail> map = JsonUtil.stringToCollect(responseResult.result);
                    return JsonUtil.toObject(JsonUtil.GsonString(map.get(detailRequest.id)), ReadDetail.class);
                } else {
                    return null;
                }
            }
        }).subscribeOn(Schedulers.newThread()).subscribe(new Observer<ReadDetail>() {
            @Override
            public void onSubscribe(@NonNull Disposable d) {
            ...
            @Override
            public void onNext(@NonNull ReadDetail news) {
                if (news != null)
